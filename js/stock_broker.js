// Stock broker core functionality
// Initialize the db
...
exports.addItem = function(item_code, item_name, item_price, item_arrivalDate, item_minRestockQty, item_qty, item_maxStockQty, item_staffCheckName) {
  ...
  db.insert(item, function(err, newDoc) {
    notification_broker.create('Stock_Broker:Event','New stock item "' + item_name + '" has been added by user'); // Register event notification log (async)
  });
};
...
exports.getItems = function(x) {
  db.find({}, function(err, docs) {
    notification_broker.create('Stock_Broker:Event','User has retrieved all stock records'); // Register event notification log (async)
...
exports.updateItemStock = function(id, newqty){
  db.update({item_code: id},{$inc: {item_qty: newqty}}, function(err, num){
    notification_broker.create('Stock_Broker:Event','SYSTEM has updated item stock level with ID: ' + id); // Register event notification log (async)
  });
}
...
exports.deleteItem = function(id) {
  db.remove({ _id: id }, {}, function(err, numRemoved) {
    notification_broker.create('Stock_Broker:Event','User has deleted the item with ID: ' + id); // Register event notification log (async)
  });
}
...
exports.updateItem = function(item_code, newdata){
  db.update({item_code: item_code}, newdata, function(err,numReplaced){
    notification_broker.create('Stock_Broker:Event','User has updated records of item with ID: ' + item_code); // Register event notification log (async) 
  });
}
...
exports.getItem = function(id, callback){
  db.find({item_code: id}, function(err,docs){
    notification_broker.create('Stock_Broker:Event','Uses has retrieved records of item with ID: '+ id ); // Register event notification log (async)
    callback(null, docs);
  });
}
